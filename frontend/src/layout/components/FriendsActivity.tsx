...
const FriendsActivity = () => {
  const { isLoading, users, fetchUsers, onlineUsers, userActivities } =
    useChatStore();
  const { user } = useUser();
  useEffect(() => {
    if (user) fetchUsers();
      ...
      <div className='h-full bg-zinc-900 rounded-lg flex flex-col'>
        <div className='p-4 flex justify-between items-center border-b border-zinc-900'>
          <div className='flex items-center gap-2'>
            ...
            <span className='animate-spin'>&times;</span>
          </div>
        </div>
      </div>
    );
      ...
      <ScrollArea className='flex-1'>
        <div className='p-4 space-y-4'>
          {users.map((user) => {
            ...
            //console.log('FriendsActivity,user', user.fullName, ':', activity);
            return (
              <div
                key={user._id}
